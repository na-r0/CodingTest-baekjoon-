...
total = []
for i in range(8):
    total.append(int(data[0][i]))
    total.append(int(data[1][i]))

phone = total
while True:
    result = []
    for i in range(len(phone)-1):
        if phone[i]+phone[i+1] < 10:
            result.append(phone[i]+phone[i+1])
        else: result.append(phone[i]+phone[i+1]-10)
    if len(phone) == 2: break
    phone = result
print(str(phone[0])+str(phone[1]))