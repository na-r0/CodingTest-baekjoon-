...
# 둘째 줄에는 입력할 명령어의 개수를 나타내는 정수 M(1 ≤ M ≤ 500,000)이 주어진다. 셋째 줄부터 M개의 줄에 걸쳐 입력할 명령어가 순서대로 주어진다. 명령어는 위의 네 가지 중 하나의 형태로만 주어진다.
...
for _ in range(M):
    ...
    if (c[0] == 'L') and (s1 != []): s2.append(s1.pop(-1))
    elif (c[0] == 'D') and (s2 != []): s1.append(s2.pop(-1))
    elif (c[0] == 'B') and (s1 != []): s1.pop(-1)
    elif c[0] == 'P': s1.append(c[1])
print(''.join(s1+list(reversed(s2)))) # 답: abcdyx