...
# 입력
# 첫째 줄에 컵의 위치를 바꾼 횟수 M이 주어지며, M은 50보다 작거나 같은 자연수이다. 둘째 줄부터 M개의 줄에는 컵의 위치를 바꾼 방법 X와 Y가 주어지며,
...
for i in range(len(data)):
    index1 = check.index(data[i][0])
    index2 = check.index(data[i][1])
    check[index1] = data[i][1]
    check[index2] = data[i][0]
